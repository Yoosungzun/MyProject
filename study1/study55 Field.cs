...
namespace TEXTRPG
{
    ...
    public class Field
    {
        Player m_pPlyaer = null;

        //몬스터
        Monster m_pMonster = null;

        //플레이어 maingame에서 생성한거 가지고오기
        //플레이어가 필드에서 전투를 하면 데미지를 입을 거고hp가 깍이겠다?
        //그럼 배운것 중에 ref 써 볼까?

        public void SetPlayer(ref Player pPlayer) { m_pPlyaer = pPlayer; }
        ...
        public void Create(string _strName, int _iHp, int _iAttack, out Monster pMonster)
        {
            ...
            pMonster.SetMonster(tMonster); //생성된 데이터 INFO 클래스 타입 이자로 데이터 세팅팅
        }
        ...
        public void Fight()
        {
            ...
            while (true)
            {
                ...
                if (iInput == 1)
                {
                    ...
                    m_pPlyaer.SetDamage(m_pMonster.GetMonster().iAttack);
                    //몬스터 데미지 주기 플레이어 공격력 넣기
                    m_pMonster.SetDamage(m_pPlyaer.GetInfo().iAttack);

                    if (m_pPlyaer.GetInfo().iHP <= 0) //플레이 체력이 0 이하이면
                    {
                        m_pPlyaer.SetHp(100);
                        break;
                    }
